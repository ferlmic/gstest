...
#' 
#' @param alterability_df_only (optionnel)
#'
#' Argument logique (*logical*) spécifiant si oui ou non seul l'ensemble des coefficients d'altérabilité trouvés dans le 
#' fichier d'altérabilité (argument `alterability_df`) doit être inclus dans le *data frame* de spécifications de problème 
#' [tsbalancing()] renvoyé. Lorsque `alterability_df_only = FALSE` (la valeur par défaut), les coefficients d'altérabilité 
#' spécifiés avec les arguments `alterSeries`, `alterTotal1` et `alterTotal2` sont combinés avec ceux trouvés dans 
#' `alterability_df` (les derniers coefficients remplaçant les premiers) et le *data frame* renvoyé contient donc les 
#' coefficients d'altérabilité pour toutes les séries composantes et de totaux de contrôle transversaux. Cet argument 
#' n'affecte pas l'ensemble des coefficients d'altérabilité des totaux temporels (associés à l'argument `alterAnnual` de 
#' [tsraking()]) qui sont inclus dans le *data frame* de spécifications de problème [tsbalancing()] renvoyé. Ce dernier 
#' contient toujours strictement ceux spécifiés dans `metadata_df` avec une valeur non manquante (non `NA`) pour la colonne 
#' `alterAnnual`.
#'
...
#' @details
#' 
#' La description précédente de l'argument `alterability_df` provient de [tsraking()]. Cette fonction (`rkMeta_to_blSpecs()`) 
#' modifie légèrement la spécification des coefficients d'altérabilité avec l'argument `alterability_df` en permettant 
#' * soit un seul enregistrement, spécifiant l'ensemble des coefficients d'altérabilité à utiliser pour toutes les périodes, 
#' * soit un ou plusieurs enregistrements avec une colonne supplémentaire nommée `timeVal` permettant de spécifier à la fois 
#' des coefficients d'altérabilité spécifiques à la période (`timeVal` n'est pas `NA`) et des coefficients génériques à 
#' utiliser pour toutes les autres périodes (`timeVal` est `NA`). Les valeurs de la colonne `timeVal` correspondent aux 
#' *valeurs de temps* d'un objet « ts » telles que renvoyées par [stats::time()], correspondant conceptuellement à 
...
######
#' Une autre différence avec [tsraking()] est que des valeurs manquantes (`NA`) sont autorisés dans le *data frame* des 
#' coefficients d'altérabilité (argument `alterability_df`) et que l'on utiliserait alors les coefficients génériques 
#' (enregistrements pour lesquels `timeVal` est `NA`) ou les coefficients par défaut (arguments `alterSeries`, `alterTotal1` 
#' et `alterTotal2`).
#'
#' Notez que à part rejeter les coefficients d'altérabilité pour les séries qui ne sont pas énumérées dans le *data frame* 
#' des métadonnées de ratissage (argument `metadata_df`), cette fonction ne valide pas les valeurs trouvées dans le 
#' *data frame* des coefficients d'altérabilité (argument `alterability_df`) ni celles trouvées dans la colonne `alterAnnual` 
#' du *data frame* des métadonnées de ratissage (argument `metadata_df`). La fonction les transfère *telles quelles* dans le 
#' *data frame* des spécifications de problème [tsbalancing()] renvoyé.
#'
#'
#' @returns
